...
import { MessageCircle, Linkedin, Github, Instagram, Globe, X } from 'lucide-react';
...
const WhatsAppButton: React.FC = () => {
  ...
  return (
    <>
      ...
      `}</style>
      <div ref={containerRef} className="fixed bottom-6 right-6 sm:right-8 z-[120]">
        {/* Toggle Button */}
        <button
          onClick={() => setShowButtons(!showButtons)}
          className="bg-indigo-500 text-white rounded-full p-4 shadow-[0_20px_50px_rgba(99,102,241,0.3)] transition-all duration-300 hover:bg-indigo-600 active:scale-95"
          aria-label={showButtons ? "Close menu" : "Open menu"}
        >
          {showButtons ? <X size={24} /> : <Globe size={24} />}
        </button>

        {/* Buttons Container - Shows when toggle is active */}
        {showButtons && (
          <div className="absolute bottom-0 right-0 flex flex-col items-end gap-4 mt-24">
